...
public class ForeignEngine implements Engine {
    ...
    public int importFile(String gpsfile) throws Engine.Err {
        try (Arena arena = Arena.ofConfined()) {
            ...
            var path_to_file = arena.allocateFrom(gpsfile);
            var path_to_storage = arena.allocateFrom(storage_path);
            int id = (int) func.invokeExact(path_to_file, path_to_storage);
            return id;
        } catch (Throwable t) {
