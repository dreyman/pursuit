...
public class ForeignEngine implements Engine {
    ...
    public int importFile(String gpsfile) throws Engine.Err {
        try (Arena arena = Arena.ofConfined()) {
            ...
            var func_ptr = libpursuit.find(import_file_func.name)
                    .orElseThrow(() -> Engine.Err.functionNotFound(import_file_func.name));
    ...
    public void recalcStats(int id, int min_speed, int max_time_gap) throws Err {
        try (Arena arena = Arena.ofConfined()) {
            var path = Path.of(lib_path);
            SymbolLookup libpursuit = SymbolLookup.libraryLookup(path, arena);
            var func_ptr = libpursuit.find(recalc_stats_func.name)
                    .orElseThrow(() -> Engine.Err.functionNotFound(recalc_stats_func.name));
    ...
    public String version() throws Engine.Err {
        try (Arena arena = Arena.ofConfined()) {
            var path = Path.of(lib_path);
            SymbolLookup libpursuit = SymbolLookup.libraryLookup(path, arena);
            var func_ptr = libpursuit.find(version_func.name)
                    .orElseThrow(() -> Engine.Err.functionNotFound(version_func.name));
